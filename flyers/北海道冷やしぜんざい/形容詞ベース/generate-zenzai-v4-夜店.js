...
// ブラッシュアップ：紺地の単調さを避けるための麻の葉風ドットテクスチャ（うっすら）
for (let row = 0; row < 16; row++) {
  for (let col = 0; col < 9; col++) {
    const dx = (row % 2 === 0) ? 0 : 0.46;
    slide.addShape('ellipse', {
      x: col * 0.92 + dx - 0.03, y: 4.7 + row * 0.46 - 0.03, w: 0.06, h: 0.06,
      fill: { color: GOLD, transparency: 82 }, line: { type: 'none' },
    });
  }
}
...
safeWriteFile(pptx, path.join(__dirname, 'zenzai-v4-夜店.pptx')).then(() => console.log('written v4'));
